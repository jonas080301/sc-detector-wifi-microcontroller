...
/** @defgroup STM32L475E_IOT01_LOW_LEVEL_Exported_Constants LOW LEVEL Exported
 * Constants
 * @{
 */
...
#define DISCOVERY_COMx_CLK_DISABLE(__INDEX__)                                  \
  do {                                                                         \
    if ((__INDEX__) == COM1) {                                                 \
...
#define NFC_I2C_TIMEOUT_STD                                                    \
  (uint32_t)8 /* I2C Time out (ms), used to call Transmit/Receive HAL          \
                 functions */
#define NFC_I2C_TIMEOUT_MAX                                                    \
  (uint32_t)200 /* I2C Time out (ms), this is the maximum time needed by M24SR \
                   to complete any command */
#define NFC_I2C_TRIALS                                                         \
  (uint32_t)1 /* In case M24SR will reply ACK failed allow to perform retry    \
                 before returning error (HAL option not used) */
...
/** @defgroup STM32L475E_IOT01_LOW_LEVEL_Exported_Functions LOW LEVEL Exported
 * Functions
 * @{
 */
