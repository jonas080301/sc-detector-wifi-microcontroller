 ...
 * @}
 */
/** @defgroup STM32L475E_IOT01_LOW_LEVEL_Private_FunctionPrototypes LOW LEVEL
...
/** @defgroup STM32L475E_IOT01_LOW_LEVEL_Private_Functions LOW LEVEL Private
 * Functions
 * @{
 */
 ...
 */
uint32_t BSP_GetVersion(void) { return __STM32L475E_IOT01_BSP_VERSION; }
 ...
 * @retval None
 */
void SENSOR_IO_Init(void) { I2Cx_Init(&hI2cHandler); }

/**
 * @brief  DeInitializes Sensors low level.
 * @retval None
 */
void SENSOR_IO_DeInit(void) { I2Cx_DeInit(&hI2cHandler); }
 ...
 */
void SENSOR_IO_Write(uint8_t Addr, uint8_t Reg, uint8_t Value) {
  I2Cx_WriteMultiple(&hI2cHandler, Addr, (uint16_t)Reg, I2C_MEMADD_SIZE_8BIT,
                     (uint8_t *)&Value, 1);
}
...
uint8_t SENSOR_IO_Read(uint8_t Addr, uint8_t Reg) {
  ...
  I2Cx_ReadMultiple(&hI2cHandler, Addr, Reg, I2C_MEMADD_SIZE_8BIT,
...
uint16_t SENSOR_IO_ReadMultiple(uint8_t Addr, uint8_t Reg, uint8_t *Buffer,
                                uint16_t Length) {
  return I2Cx_ReadMultiple(&hI2cHandler, Addr, (uint16_t)Reg,
 ...
 * @retval None
 */
void SENSOR_IO_Delay(uint32_t Delay) { HAL_Delay(Delay); }
 ...
 * @retval None
 */
void NFC_IO_DeInit(void) { I2Cx_DeInit(&hI2cHandler); }
...
uint16_t NFC_IO_ReadMultiple(uint8_t Addr, uint8_t *pBuffer, uint16_t Length) {
  ...
  /* Before calling this function M24SR must be ready: check to detect potential
   * issues */
  ...
  if (HAL_I2C_Master_Receive(&hI2cHandler, Addr, (uint8_t *)pBuffer, Length,
                             NFC_I2C_TIMEOUT_STD) != HAL_OK) {
    return NFC_I2C_ERROR_TIMEOUT;
  }
...
uint16_t NFC_IO_WriteMultiple(uint8_t Addr, uint8_t *pBuffer, uint16_t Length) {
  ...
  if (HAL_I2C_Master_Transmit(&hI2cHandler, Addr, (uint8_t *)pBuffer, Length,
                              NFC_I2C_TIMEOUT_STD) != HAL_OK) {
    return NFC_I2C_ERROR_TIMEOUT;
  }
...
uint16_t NFC_IO_IsDeviceReady(uint8_t Addr, uint32_t Trials) {
  ...
  do {
    status =
        HAL_I2C_IsDeviceReady(&hI2cHandler, Addr, Trials, NFC_I2C_TIMEOUT_STD);
    currenttick = HAL_GetTick();
  } while (((currenttick - tickstart) < NFC_I2C_TIMEOUT_MAX) &&
           (status != HAL_OK));
...
void NFC_IO_ReadState(uint8_t *pPinState) {
  *pPinState =
      (uint8_t)HAL_GPIO_ReadPin(NFC_GPIO_GPO_PIN_PORT, NFC_GPIO_GPO_PIN);
}
 ...
 */
void NFC_IO_RfDisable(uint8_t PinState) {
  HAL_GPIO_WritePin(NFC_GPIO_RFDISABLE_PIN_PORT, NFC_GPIO_RFDISABLE_PIN,
