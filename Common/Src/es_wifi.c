...
static uint8_t Hex2Num(char a) {
  if (a >= '0' && a <= '9') { /* Char is num */
    return a - '0';
             ...
             a <= 'f') { /* Char is lowercase character A - Z (hex) */
    return (a - 'a') + 10;
  } else if (a >= 'A' &&
             a <= 'F') { /* Char is uppercase character A - Z (hex) */
    return (a - 'A') + 10;
  }
...
static void AT_ParseAP(char *pdata, ES_WIFI_APs_t *APs) {
  ...
  while ((ptr != NULL) && (APs->nbr < ES_WIFI_MAX_DETECTED_AP)) {
    ...
    case 1:
      ptr[strlen(ptr) - 1] = 0;
      strncpy((char *)APs->AP[APs->nbr].SSID, ptr + 1,
              sizeof(APs->AP[APs->nbr].SSID) - 1);
  ...
  while (ptr != NULL) {
    ...
    case 7:
      ParseIP(ptr, NetSettings->Gateway_Addr,
              sizeof(NetSettings->Gateway_Addr));
      break;
  ...
  while (ptr != NULL) {
    ...
    case 1:
      ParseIP(ptr, TransportSettings->Local_IP_Addr,
              sizeof(TransportSettings->Local_IP_Addr));
      break;
    ...
    case 3:
      ParseIP(ptr, TransportSettings->Remote_IP_Addr,
              sizeof(TransportSettings->Remote_IP_Addr));
      break;
...
static ES_WIFI_Status_t AT_RequestReceiveData(ES_WIFIObject_t *Obj,
  ...
  if ((Obj->fops.IO_Send != NULL) && (Obj->fops.IO_Receive != NULL)) {
    ...
    if (Obj->fops.IO_Send(cmd, (uint16_t)strlen((char *)cmd), Obj->Timeout) >
        0) {
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_GetProductName(ES_WIFIObject_t *Obj,
                                        uint8_t *productName,
                                        uint8_t ProductNameLength) {
  if (ProductNameLength > 0) {
    strncpy((char *)productName, (char *)Obj->Product_Name,
            ProductNameLength - 1);
 ...
 */
ES_WIFI_Status_t
ES_WIFI_RegisterBusIO(ES_WIFIObject_t *Obj, IO_Init_Func IO_Init,
                      IO_DeInit_Func IO_DeInit, IO_Delay_Func IO_Delay,
...
ES_WIFI_Status_t ES_WIFI_ListAccessPoints(ES_WIFIObject_t *Obj,
  ...
  if ((Obj->fops.IO_Send != NULL) && (Obj->fops.IO_Receive != NULL)) {
    ...
    if (ArrayTo32bit(version) >= UPDATED_SCAN_PARAMETERS_FW_REV) {
      ...
      send_len = Obj->fops.IO_Send(
          Obj->CmdData, (uint16_t)strlen((char *)Obj->CmdData), Obj->Timeout);
      ...
      if (send_len == 5) {
        ...
        do {
          int16_t recv_len =
              Obj->fops.IO_Receive(Obj->CmdData, cmd_data_size, Obj->Timeout);
          ...
          send_len = Obj->fops.IO_Send(Obj->CmdData,
                                       (uint16_t)strlen((char *)Obj->CmdData),
                                       Obj->Timeout);
        } while (send_len == 3);
      }
...
ES_WIFI_Status_t ES_WIFI_ActivateAP(ES_WIFIObject_t *Obj,
                                    const ES_WIFI_APConfig_t *ApConfig) {
  ES_WIFI_Status_t ret;
  ...
  ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
  if (ret == ES_WIFI_STATUS_OK) {
    snprintf((char *)Obj->CmdData, cmd_size, "A1=%c\r",
    ...
    if (ret == ES_WIFI_STATUS_OK) {
      ...
      if (ret == ES_WIFI_STATUS_OK) {
        ...
        ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
        if (ret == ES_WIFI_STATUS_OK) {
          snprintf((char *)Obj->CmdData, cmd_size, "AT=%d\r",
  ...
  do {
    ...
    if (AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData) !=
      ...
      ParseMAC((char *)ptr, Obj->APSettings.MAC_Addr,
               sizeof(Obj->APSettings.MAC_Addr));
      ptr = strtok(NULL, " ");
      ptr = strtok(NULL, "\r");
      ParseIP((char *)ptr, Obj->APSettings.IP_Addr,
              sizeof(Obj->APSettings.IP_Addr));
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_SetMACAddress(ES_WIFIObject_t *Obj,
                                       const uint8_t *mac) {
  ES_WIFI_Status_t ret;

  LOCK_WIFI();

  sprintf((char *)Obj->CmdData, "Z4=%X:%X:%X:%X:%X:%X\r", mac[0], mac[1],
...
ES_WIFI_Status_t ES_WIFI_ResetModule(ES_WIFIObject_t *Obj) {
  ...
  ret = Obj->fops.IO_Send(Obj->CmdData, strlen((char *)Obj->CmdData),
                          Obj->Timeout);
...
ES_WIFI_Status_t ES_WIFI_OTA_Upgrade(ES_WIFIObject_t *Obj, uint8_t *link) {
  ...
  snprintf((char *)Obj->CmdData, cmd_size, "Z0=%d\r%s", strlen((char *)link),
           (char *)link);
  ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_SetUARTBaudRate(ES_WIFIObject_t *Obj,
                                         uint16_t BaudRate) {
  ES_WIFI_Status_t ret;
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_GetUARTConfig(ES_WIFIObject_t *Obj,
                                       ES_WIFI_UARTConfig_t *pconf) {
  ES_WIFI_Status_t ret;
  LOCK_WIFI();
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_GetSystemConfig(ES_WIFIObject_t *Obj,
                                         ES_WIFI_SystemConfig_t *pconf) {
  ES_WIFI_Status_t ret;
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_Ping(ES_WIFIObject_t *Obj, const uint8_t *address,
...
ES_WIFI_Status_t ES_WIFI_DNS_LookUp(ES_WIFIObject_t *Obj, const char *url,
                                    uint8_t *ipaddress, uint8_t IpAddrLength) {
  ES_WIFI_Status_t ret;
  char *ptr;
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_StartAWSClientConnection(ES_WIFIObject_t *Obj,
                                                  ES_WIFI_AWS_Conn_t *conn) {
  ES_WIFI_Status_t ret;
  LOCK_WIFI();
 ...
 * @retval Operation Status.
 */
ES_WIFI_Status_t ES_WIFI_CloseServerConnection(ES_WIFIObject_t *Obj,
                                               uint8_t socket) {
  ES_WIFI_Status_t ret;
...
ES_WIFI_Status_t ES_WIFI_SendDataTo(ES_WIFIObject_t *Obj, uint8_t Socket,
                                    const uint8_t *pdata, uint16_t Reqlen,
                                    uint16_t *SentLen, uint32_t Timeout,
  ...
  if (ret == ES_WIFI_STATUS_OK) {
    sprintf((char *)Obj->CmdData, "P2=%d\r",
            /*LocalPort*/ 56830); // WARN: Does not work!
    ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
  }

  // ? Are we sure that the Firmware can change the packet destination without
  // stopping the socket?
  if (ret == ES_WIFI_STATUS_OK) {
  ...
  if (ret == ES_WIFI_STATUS_OK) {
    sprintf((char *)Obj->CmdData, "P3=%d.%d.%d.%d\r", IPaddr[0], IPaddr[1],
            IPaddr[2], IPaddr[3]);
    ret = AT_ExecuteCommand(Obj, Obj->CmdData, Obj->CmdData);
  }
    ...
    if (ret == ES_WIFI_STATUS_OK) {
      ...
      if (ret == ES_WIFI_STATUS_OK) {
        ...
        if (ret == ES_WIFI_STATUS_OK) {
          ...
          ret = AT_RequestReceiveData(Obj, Obj->CmdData, (char *)pdata, Reqlen,
                                      Receivedlen);
          if (ret != ES_WIFI_STATUS_OK) {
...
ES_WIFI_Status_t ES_WIFI_ReceiveDataFrom(ES_WIFIObject_t *Obj, uint8_t Socket,
                                         uint8_t *pdata, uint16_t Reqlen,
                                         uint16_t *Receivedlen,
                                         uint32_t Timeout, uint8_t *IPaddr,
                                         uint8_t IpAddrLength,
                                         uint16_t *pPort) {
  uint32_t wkgTimeOut;
...
ES_WIFI_Status_t ES_WIFI_StoreCreds(ES_WIFIObject_t *Obj,
                                    ...
                                    uint8_t credSet, uint8_t *ca,
                                    uint16_t caLength, uint8_t *certificate,
                                    uint16_t certificateLength, uint8_t *key,
                                    uint16_t keyLength) {
  ES_WIFI_Status_t ret = ES_WIFI_STATUS_ERROR;
  ...
  if (ret == ES_WIFI_STATUS_OK) {
    ...
    ret = AT_RequestSendData(Obj, Obj->CmdData, (uint8_t *)ca, caLength,
                             Obj->CmdData);

    if (ret == ES_WIFI_STATUS_OK) {
      /* Store device certificate. */
      sprintf((char *)Obj->CmdData, "PG=%d,1,%04d\r", credSet,
              certificateLength);
      ret = AT_RequestSendData(Obj, Obj->CmdData, certificate,
                               certificateLength, Obj->CmdData);

      if (ret == ES_WIFI_STATUS_OK) {
        /* Store device key. */
        sprintf((char *)Obj->CmdData, "PG=%d,2,%04d\r", credSet, keyLength);
        ret =
            AT_RequestSendData(Obj, Obj->CmdData, key, keyLength, Obj->CmdData);
      }
    }
  }
...
ES_WIFI_Status_t ES_WIFI_StoreCertificate(ES_WIFIObject_t *Obj,
                                          ES_WIFI_CredsFunction_t credsFunction,
                                          uint8_t credSet, uint8_t *certificate,
  ...
  if (ret == ES_WIFI_STATUS_OK) {
    ...
    ret = AT_RequestSendData(Obj, Obj->CmdData, certificate, certificateLength,
                             Obj->CmdData);
  }
